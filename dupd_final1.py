...
$\small\text{1. Importación de las librerías necesarias}$


$\text{No se instalará ninguna línea [como !pip install ...] en el código, ya que no son válidas en el entorno de [Streamlit Cloud].}$
$\text{Razón por lo cual se ha modificado el título de [Instalación] a [Importación].}$
"""
...
# Guardando la 'data' en un archivo CSV
df.to_csv('video_sentiments.csv')
...
"""$\small\text{6.3. Creación del Gráfico de Caja (Boxplot) de Sensaciones}$"""
